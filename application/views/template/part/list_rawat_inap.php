      ...
      <div class="box box-info">
        ...
        <div class="box-body">
			...
			foreach($all as $row) {
				...
				if($row->ke=='9')
				{				
					$btn_cetak = "<button class='btn btn-danger btn-xs btn-block' onclick='proses_ranap($row->id_kunjungan)'>Proses</button>";
				}else{
					$btn_cetak = "<button class='btn btn-primary btn-xs btn-block' onclick='print_rawat_inap($row->id_kunjungan)'>Cetak</button>";
				}
...
function proses_ranap(id_kunjungan)
{
	eksekusi_controller("index.php/c_rawat_inap/form_rawat_inap/"+id_kunjungan);
}
